...
namespace MUNIA.Util
{
    public sealed class TimeStamp
    {
        ...
        public static TimeStamp Now
            => new TimeStamp(new TimeSpan((long)(qpc.Elapsed.Ticks / PersistentDrift)));
        ...
        private static void RefreshDrift()
        {
            while (true)
            {
                ...
                if (times.Count >= 5)
                {
                    var averageDifference = (long)times.Average();
                    lastQPCTime = qpcTime;
                    lastNTPTime = new DateTime(qpcTime.Ticks + averageDifference, DateTimeKind.Utc);
